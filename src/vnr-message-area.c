...
G_DEFINE_TYPE (VnrMessageArea, vnr_message_area, GTK_TYPE_BOX);
...
vnr_message_area_initialize(VnrMessageArea * msg_area)
{
    ...
    gtk_box_append(GTK_BOX(msg_area), msg_area->hbox);
    gtk_widget_set_margin_start(msg_area->hbox, 7);
    gtk_widget_set_margin_end(msg_area->hbox, 7);
    gtk_widget_set_margin_top(msg_area->hbox, 7);
    gtk_widget_set_margin_bottom(msg_area->hbox, 7);
    ...
    msg_area->button_box = gtk_box_new(GTK_ORIENTATION_VERTICAL, 0);
    gtk_box_pack_end(GTK_BOX (msg_area->hbox), msg_area->button_box,
                     FALSE, FALSE, 0);

    msg_area->user_button = gtk_button_new();
    gtk_box_append(GTK_BOX(msg_area->button_box), msg_area->user_button);
    ...
    gtk_box_append(GTK_BOX(msg_area->button_box), msg_area->cancel_button);
                             ...
                             const char *message, gboolean close_image)
{
    ...
    if(critical)
        gtk_image_set_from_icon_name(GTK_IMAGE(msg_area->image), "dialog-error");
    else
        gtk_image_set_from_icon_name(GTK_IMAGE(msg_area->image), "dialog-information");
                       ...
                       const char *message, gboolean close_image)
{
    ...
    gtk_widget_show(GTK_WIDGET(msg_area->hbox));
    gtk_widget_show(GTK_WIDGET(msg_area));
